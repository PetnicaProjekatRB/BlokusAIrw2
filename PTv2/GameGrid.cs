...
namespace BlokusDll
{
    public class GameGrid
    {
        ...
        private bool FreeSlow(int x, int y, Player pl)
        {
            return (
                    !Covered(x, y) &&
                    ((x >= 13) || !Covered(x + 1, y, pl)) &&
                    ((x <= 0) || !Covered(x - 1, y, pl)) &&
                    ((y >= 13) || !Covered(x, y + 1, pl)) &&
                    ((y <= 0) || !Covered(x, y - 1, pl))
                   );
        }

        public bool isLiberty(int x, int y, Player pl)
        {
            return GetLibertyCache(x, y, pl) != 0;
        }
        ...
        {
            int c = p.coords.Length;
            if ((x > c - 1) && (x < 13 - c) && (y < 13 - c) && (y > c - 1)) 
                return true;
            foreach (var s in p.coords)
            {
                var ax = s[0] + x;
